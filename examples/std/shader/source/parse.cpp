...
int main(int, char*[]) {
	static constexpr std::string_view vx_shader_main_file = "shaders/sprite.vxglsl";
	static constexpr std::string_view px_shader_main_file = "shaders/sprite.pxglsl";
	static constexpr std::span<const char> vx_shader_main = phd::embed<char>(vx_shader_main_file);
	...
	std::cout << "[[Vertex Shader - " << vx_shader.size() << " bytes]]\n";
	std::cout << vx_shader.data();
	std::cout << std::endl;

	std::cout << "[[Pixel Shader - " << px_shader.size() << " bytes]]\n";
