...
template <typename Action>
consteval void preprocess_shader_lex_include(std::string_view file_name, std::string_view directive, Action& action,
                                             int depth) {
...
consteval void preprocess_shader(std::string_view file_name, std::span<const char> data, Action& action, int depth) {
	...
	for (std::size_t i = 0; i < shader_text.size();) {
		...
		if (currentc == '#') {
			...
			if (end_of_directive_index == std::string_view::npos) {
